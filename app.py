...
st.set_page_config(page_title="JC Contabilidade - Folha de salários Postos Buffon", layout="wide")
...
with st.container(border=True):
    ...
    if uploaded_file is not None:
        try:
            ...
            # Lê o CSV
            df = pd.read_csv(
                io.StringIO(content), delimiter=delimiter, engine="python", on_bad_lines="skip",header=None
            )   

            # Limpeza básica
            df = df.dropna(axis=1, how="all")
            df = df.dropna(axis=0, how="all")
            df.columns = ["col_1", "col_2", "col_3", "contabilidade", "col_5", "col_6", "col_7", "codigo", "descricao","valor","nome","divisor","filial","cnpj","digito"] + list(df.columns[15:])
            ...
            if df.shape[1] > 9:
                ...
                df["Informações_Complementares"] = colunas_para_juntar.astype(str).apply(
                    lambda x: " ".join(
                        [
                ...
                df = pd.concat([primeiras_colunas, df["Informações_Complementares"]], axis=1)
            else:
                st.warning("O arquivo possui menos de 8 colunas. Nenhuma junção foi feita.")
            ...
            # Extrai os campos
            extraidos = df["Informações_Complementares"].apply(extrair_info).apply(pd.Series)
            df_final = pd.concat(
            ...
            codigos_excluir = ["3101", "4000", "4006", "4013", "4017", "4019", "4025", "20069"]

            if "1.4" in df_final.columns:
                df_final = df_final[~((df_final["digito"] == "P") & (df_final["1.4"].astype(str).isin(codigos_excluir)))]
...
if uploaded_file is not None and 'df_final' in locals():
    with st.container(border=True):
        ...
        with tabs[0]:
            ...
            df_table = df_final[["codigo", "descricao","valor","nome","filial","cnpj","digito"]]
            digito_map = {
                "P": "Provento",
                "D": "Desconto",
            }
            df_table["digito"] = df_table["digito"].apply(lambda x: digito_map.get(x, x))
            df_table = df_table.sort_values(
                by=["filial", "digito"],
                key=lambda col: (
                    col.map(sort_filial) if col.name == "filial" 
            ...
            with col1:
                filiais_opcoes = sorted(df_table["filial"].dropna().unique(), key=sort_filial)
            ...
            with col2:
                tipos_opcoes = sorted(df_table["digito"].dropna().unique(), key=sort_tipo)
            ...
            if len(tipos_selecionados) > 0:
                df_filtrado = df_filtrado[df_filtrado["digito"].isin(tipos_selecionados)]
        ...
        with tabs[1]:
            ...
            todos_codigos = sorted(df_final["codigo"].astype(str).dropna().unique(), key=lambda x: int(x))
            ...
            df_final["codigo_desc"] = df_final["codigo"].astype(str) + " – " + df_final["descricao"].astype(str)

            codigo_map = dict(zip(df_final["codigo_desc"], df_final["codigo"].astype(str)))
            ...
            codigos_P_default_fmt = [c for c in codigos_P_formatados if codigo_map[c] in codigos_P_padrao]
            codigos_D_default_fmt = [c for c in codigos_D_formatados if codigo_map[c] in codigos_D_padrao]

                    
            with st.expander("Selecionar códigos de proventos e descontos para ajuste do total líquido"):
                st.write(
            ...
            with colP:
                codigos_P_select_fmt = st.multiselect(
                    "Selecionar códigos de proventos:",
                    options=codigos_P_formatados,
                    default=codigos_P_default_fmt,
                )
                # Converter "código – descrição" → código
                codigos_P_selecionados = [codigo_map[x] for x in codigos_P_select_fmt]

            with colD:
                codigos_D_select_fmt = st.multiselect(
                    "Selecionar códigos de descontos:",
                    options=codigos_D_formatados,
                    default=codigos_D_default_fmt,
                )
                # Converter "código – descrição" → código
                codigos_D_selecionados = [codigo_map[x] for x in codigos_D_select_fmt]

            st.markdown("<hr style='padding:4px 0 0 0;margin:0;'>", unsafe_allow_html=True)

            df_codigos_especiais = (
                df_final[
                    ((df_final["digito"] == "P") & (df_final["codigo"].astype(str).isin(codigos_P_selecionados)))
                    |
                    ((df_final["digito"] == "D") & (df_final["codigo"].astype(str).isin(codigos_D_selecionados)))
                ]
                .groupby("filial", as_index=False)["valor_num"]
                .sum()
            )

            df_codigos_especiais.rename(columns={"valor_num": "total_codigos_especiais"}, inplace=True)

            df_soma = df_soma.merge(df_codigos_especiais, on="filial", how="left")

            df_soma["total_codigos_especiais"] = df_soma["total_codigos_especiais"].apply(
                lambda x: f"R${x:,.2f}".replace(",", "X").replace(".", ",").replace("X", ".")
                if pd.notnull(x)
                else "R$0,00"
            )
            ...
            df_soma["total_proventos_num"] = df_soma["total_proventos"].str.replace("R$", "").str.replace(".", "").str.replace(",", ".").astype(float)
            df_soma["total_codigos_especiais_num"] = df_soma["total_codigos_especiais"].str.replace("R$", "").str.replace(".", "").str.replace(",", ".").astype(float)

            # Novo total líquido = Proventos – Códigos Selecionados
            df_soma["total_liquido_num"] = df_soma["total_proventos_num"] - df_soma["total_codigos_especiais_num"]
            ...
            df_soma = df_soma.sort_values(
                by="filial",
                key=lambda col: col.map(sort_filial)
            )

            filiais_opcoes = sorted(df_soma["filial"].dropna().unique(), key=sort_filial)
            filiais_selecionadas = st.multiselect(
                    "Filtrar por Filial:",
                    filiais_opcoes,
                    default=None,
            ...
            colC.metric("Soma total das reduções selecionadas", fmt(soma_total_codigos))
